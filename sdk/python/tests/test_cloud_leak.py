"""Regression: a cloud machine that fails to become ready must NOT leak — the SDK
must delete the just-created machine before surfacing the error."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/v1/machines":
            ...
            self.rfile.read(n)
            return self._send(201, json.dumps({"id": MID, "name": "leaky", "state": "stopped"}).encode())
...
def main() -> int:
    ...
    check("create() raised when the machine entered error state", threw, code)
